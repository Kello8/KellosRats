...
public class UnstableRatEntity extends TamableAnimal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    public UnstableRatEntity(EntityType<? extends TamableAnimal> entityType, Level level) {
        super(entityType, level);
    }

    private static final Ingredient FOOD = Ingredient.of(Items.IRON_INGOT, Items.GOLD_INGOT, Items.DIAMOND, Items.EMERALD);
    ...
    protected void playStepSound(BlockPos p_28301_, BlockState p_28302_) {
        this.playSound(SoundEvents.CHICKEN_STEP, 0.15F, 0.9F);
    }
    ...
    public static AttributeSupplier setAttributes() {
        return Animal.createMobAttributes()
                ...
                .add(Attributes.ATTACK_SPEED, 2.4f)
    ...
    public boolean doHurtTarget(Entity p_32892_) {
        ...
        if (flag && this.getMainHandItem().isEmpty() && p_32892_ instanceof LivingEntity) {
            ...
            ((LivingEntity)p_32892_).addEffect(new MobEffectInstance(MobEffects.WITHER, 600 * (int)f), this);
            this.explodeRat();
        }
    ...
    private <T extends GeoAnimatable> PlayState attackPredirate(AnimationState<T> tAnimationState) {
        if(this.swinging && tAnimationState.getController().getAnimationState().equals(AnimationController.State.STOPPED)) {
            ...
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.unstable_rat.attack", Animation.LoopType.PLAY_ONCE));
            this.swinging = false;
        }
    ...
    protected ItemStack getRat() {
        return new ItemStack(ModItems.UNSTABLE_RAT.get());
    }
    ...
    private <T extends GeoAnimatable> PlayState predirate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.unstable_rat.walk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.unstable_rat.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
